...
async def next_page(bot, query):
    ...
    if int(req) not in [query.from_user.id, 0]:
        return await query.answer("കൌതുകും ലേശം കൂടുതൽ ആണല്ലേ 👀..!!\nRequest ur own files, then click here🙂", show_alert=True)
    try:
        offset = int(offset)
    except:
        offset = 0

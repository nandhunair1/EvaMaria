...
async def next_page(bot, query):
    ...
    if int(req) not in [query.from_user.id, 0]:
        return await query.answer(f"😢 Hey, {query.from_user.first_name}! Search Your Own File, Don't Click Others Results 👀", show_alert=True)
    try:
        offset = int(offset)
    except:
        offset = 0
...
async def advantage_spoll_choker(bot, query):
    ...
    if k == False:
        ...
        if files:
            ...
            k = await query.message.edit('<b>This Movie or Series Not Found In DataBase.</b>\n\n<b>Theater Print Not Available Here..!</b>')
...
async def cb_handler(client: Client, query: CallbackQuery):
    ...
    if query.data.startswith("file"):
        ...
        if not files_:
            return await query.answer(f'⚠️ Hey, {query.from_user.first_name}! No such file exist.')
        files = files_[0]
        ...
        buttons = [[
            ...
            InlineKeyboardButton("♻️ ⒼⓇⓄⓊⓅ ♻️", url="https://t.me/tvseriezzz_group"),
...
async def advantage_spell_chok(msg):
    ...
    gs_parsed = []
    if not g_s:
        k = await msg.reply("<b>I couldn't find any Movie or Series in that name.</b>")
